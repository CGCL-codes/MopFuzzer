...
//		if ("main".equals(methodName)) return;
		...
		boolean isStatic = isStatic(methodDeclarationTree);

		Method m = new Method(className, mm, returnType, methodName, parameterTypes, isConstructor, isStatic);
		SetUtils.addToMap(this.methods, className, m);
	}

	private boolean isStatic(ITree methodDeclarationTree) {
		List<ITree> children = methodDeclarationTree.getChildren();
		for (ITree child : children) {
			if (Checker.isModifier(child.getType())) {
				if ("static".equals(child.getLabel())) {
					return true;
				}
			}
		}
	...
	private List<String> readParameterTypes(String methodDeclarationLabel) {
		...
		if (methodDeclarationLabel.endsWith("@@Argus:null")) {
			...
			if (expIndex > 0) {
				argus = argus.substring(0, expIndex);
			}
			if (argus.equals("null")) {
				return parameterTypes;
			}
