...
//            }
    ...
    public static Pair<Integer, Boolean> identifyRelatedStatements(ITree suspStmtTree, List<String> vars, int endPosition) {
        List<String> varNames = new ArrayList<>();
        boolean hasReturn = false;
        varNames.addAll(vars);
        ...
        for (int i = 0; i < peerStmts.size(); i++) {
            ...
            if (isFollowingPeerStmt) {
                ...
                if (Checker.isVariableDeclarationStatement(peerStmtType)
                    ...
                    varNames.addAll(identifyVariableNames(peerStmt));
                }
                ...
                if (peerStmt == suspStmtTree) {
                    ...
                    if (Checker.isVariableDeclarationStatement(suspStmtType) ||
                        ...
                        varNames.addAll(identifyVariableNames(suspStmtTree));
                    }
                }
            }
        }
    ...
    public static List<String> identifyVariableNames(ITree stmtAst) {
        ...
        if (Checker.isVariableDeclarationStatement(stmtAstType)) {
            ...
            for (int index = 1; index < children.size(); index++) {
                ITree child = children.get(index);
                vars.add(child.getChild(0).getLabel());
            }
            return vars;
        } else if (Checker.isExpressionStatement(stmtAstType)) {
            return List.of(children.get(0).getChild(0).getLabel());
            ...
            for (int index = 0, size = children.size(); index < size; index++) {
                if (!Checker.isModifier(children.get(index).getType())) {
                    return List.of(children.get(index + 1).getLabel());
                }
            }
        }
